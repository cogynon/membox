...
> **Living tracker.** This file is the canonical known-issues list for `remembox`.
...
>
> Full-repo evaluation of `remembox` v0.2.0. Every module read end-to-end;
...
**File:** `src/reremembox/memory.py` — `update_episode`
...
**File:** `src/reremembox/reflection.py` — `RuleBasedReflectionExtractor.extract`
...
**File:** `src/reremembox/memory.py` — `context` (section 1) + `src/reremembox/semantic.py` — `about`
...
**File:** `src/reremembox/consolidation.py` — `consolidate`
...
**File:** `src/reremembox/consolidation.py` — `RuleBasedConsolidator.extract_facts`
...
**File:** `src/reremembox/forgetting.py` — `forget` + `src/reremembox/episodic.py` — `mark_consolidated`
...
**File:** `src/reremembox/memory.py` — `maintain` (order: summarize → forget)
...
### 9. ✅ 🔴 `Reremembox.reflect()` has no `now` param → `maintain(now=...)` is non-deterministic
**File:** `src/reremembox/memory.py` — `reflect` + `src/reremembox/reflection.py` — `RuleBasedReflectionExtractor.extract`
...
**File:** `src/reremembox/models.py` — `Procedure.to_dict`
...
**File:** `src/reremembox/memory.py` — `context`
...
**File:** `src/reremembox/embedding_store.py` — `stats`
...
**Files:** `src/reremembox/episodic.py` — `search`, `src/reremembox/semantic.py` — `search`, `src/reremembox/procedural.py` — `match` (substring)
...
**File:** `src/reremembox/memory.py` — `reflect`
...
**File:** `src/reremembox/episodic.py` — `iter_all`
...
**File:** `src/reremembox/consolidation.py`
...
**Files:** `src/reremembox/memory.py` — `annotate_episode`, `src/reremembox/reflection.py`
...
**File:** `src/reremembox/semantic.py` — `find` / `about`
...
**File:** `src/reremembox/forgetting.py` — `forget`
...
**File:** `src/reremembox/procedural.py` — `match`
...
**File:** `src/reremembox/embedding_store.py` — `similarity_by_vector`
...
**File:** `src/reremembox/embeddings.py`
**Verified:** Only referenced in its own docstring + `tests/test_embeddings.py`. The production path uses `embedding_store.EmbeddingStore`. Two parallel embedding implementations; `EmbeddingIndex` is in-memory-only and never wired into `Reremembox`.
...
**Files:** `src/reremembox/episodic.py` — `by_time_range`, `delete_before`; `src/reremembox/embedding_store.py` — `delete_before`
...
**File:** `src/reremembox/memory.py` — `maintain`
...
**File:** `src/reremembox/consolidation.py` + `src/reremembox/memory.py` — `maintain`
...
**Status:** Extracted to `src/reremembox/tokens.py` (`tokenize`, `tokenize_list`); both modules import it.
...
**Files:** `src/reremembox/memory.py` — `_estimate_tokens`, `src/reremembox/summarization.py` — `estimate_tokens` (two copies!)
...
**File:** `src/reremembox/retrieval.py` — `recall`
...
**File:** `src/reremembox/reflection.py`
...
**File:** `src/reremembox/models.py` — `Procedure.from_row`
